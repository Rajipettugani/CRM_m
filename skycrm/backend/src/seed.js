...
const run = async () => {
  ...
  const users = [
    { name:'Admin', email:'admin@test.com', pass:'password@123', role:'Admin' },
    { name:'Sales Head', email:'manager@test.com', pass:'manager@123', role:'Sales Head' },
    { name:'Team Lead', email:'lead@test.com', pass:'lead@123', role:'Sales Team Lead' },
    { name:'Sales Rep', email:'rep@test.com', pass:'rep@123', role:'Sales Representative' },
  ];
  ...
  if (!team) {
    team = await Team.create({
      name: 'Alpha Team',
      manager: created['Sales Head']._id,
      lead: created['Sales Team Lead']._id,
      members: [created['Sales Representative']._id]
    });
  }
  ...
  for (const s of samples) {
    ...
    if (!exists) {
      await Lead.create({
        ...
        assignedTo: created['Sales Representative']._id,
        teamId: team._id,
        status: statusByName[s.status]._id,
        history: [{ status: statusByName[s.status]._id, by: created['Sales Head']._id, at: new Date() }],
        notes: []
      });
    }
  }
